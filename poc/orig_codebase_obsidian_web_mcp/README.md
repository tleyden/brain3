...
The frontmatter index is built once when the server process starts and stays resident for the lifetime of the process. Stateless MCP HTTP requests reuse that in-memory index; they do not rebuild it per request or per session.
...
- The frontmatter index watches for filesystem changes via `watchdog` and updates automatically when Sync, Obsidian, or MCP writes add, modify, or delete markdown files

On large vaults or low-CPU hosts, the initial process startup can still spend noticeable time building the first frontmatter snapshot. That work now happens only at server boot, not on each MCP request.
