...
- Native audio transcription intentionally moves untrusted binary audio parsing into the privileged Rust gateway process. This is an accepted trade-off for host-side Metal/CPU inference, but decoder or `whisper.cpp` memory-safety issues would affect the gateway process rather than the sandboxed MCP container.
- The native transcription `download_url` is not host-allowlisted. Size limits and audio decoding reduce impact, but this remains an accepted SSRF-adjacent egress surface for authorized clients.
...
| TUI connection-card credential display | Local shoulder-surfing credential exposure | Hardened after scan | As of the 2026-06-26 TUI update, successful first-run startup lands on `RuntimeStatus` instead of immediately showing the connection card. `ConnectionCard` masks `client_secret`, password, and local MCP bearer token by default, with an explicit `[s]` reveal/hide toggle. Verified by focused TUI regression tests and full `cargo test`. |
...
- Are there any remaining local-only setup/TUI flows that unintentionally expose credentials or change tunnel posture after the validated gateway defaults are fixed?
  - 2026-06-26 update: the known `ConnectionCard` credential-display issue was hardened by landing successful first-run startup on `RuntimeStatus`, masking `client_secret`, password, and local MCP bearer token by default, and requiring an explicit `[s]` toggle to reveal them. The Summary wizard screen still intentionally displays credentials while the user is actively configuring them.
  - Follow-up prompt: Perform a dedicated local-UI review of `apps/gateway/src/main.rs`, `apps/gateway/src/setup_tui.rs`, `apps/gateway/src/tui/app.rs`, `apps/gateway/src/tui/screens.rs`, and `apps/gateway/src/tui/state.rs`, focusing on remaining credential display, log mirroring, and tunnel-mode transitions.
