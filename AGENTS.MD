...
- For risky changes that touch the gateway server, MCP proxy, container startup, or vault-tools, run the E2E smoke test to verify end-to-end behaviour: `docker build -f ./brain3-mcp-vault-tools/Containerfile -t brain3-mcp-vault-tools:e2e-local ./brain3-mcp-vault-tools && cargo test -p brain3 --test e2e_smoke --features e2e -- --nocapture`
