...
- **Tampered release binaries** — Brain3 releases now publish signed checksum metadata (`SHA256SUMS` and `SHA256SUMS.sig`), but `install.sh` does not yet enforce validation automatically, so a compromised distribution path could still serve a modified installer or bypass metadata checks for default installs.
