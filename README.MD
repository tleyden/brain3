...
Brain3 lets you interact with your local markdown vault through Claude, ChatGPT or any AI assistant that supports the Model Context Protocol (MCP) standard.  It runs as a remote MCP server that runs on your own machine (see [Privacy & Security](#privacy--security) for why).

**Why This Exists**

I built this so I could access my Obsidian markdown vault directly from the Claude mobile app, and in particular so I could access it using voice dictation.  

The nice thing about integrating tightly with AI is that you can save ideas to your vault mid-conversation and pull stored context back in whenever you need it, without switching apps.
...
### Install + run the Brain3 Terminal UI (TUI) app
...
and it should an interactive setup wizard.
